...
const logout = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    ...
    return res.sendStatus(204);   //forbidder
  }
...
const userCart = asyncHandler(async (req, res) => {
  const { productId, shade, quantity, price } = req.body;
  const { _id } = req.user;
  validateMongoDbId(_id);
  try{
...
const createOrder = asyncHandler(async (req, res) => {
  const { deliveryInfo, orderItems, totalPrice, totalPriceAfterDiscount, paymentInfo} = req.body;
  const { _id } = req.user;
  try {
    const order = await Order.create({
      deliveryInfo, orderItems, totalPrice, totalPriceAfterDiscount, paymentInfo, user:_id
    })
    res.json({
      order,
      success: true
...
const getMyOrders = asyncHandler(async (req, res) => {
  const { _id } = req.user;
  try{
    const orders = await Order.find({user:_id}).populate("user").populate("orderItems.product").populate("orderItems.shade")
    res.json({
      orders
    })
  } catch(error){
