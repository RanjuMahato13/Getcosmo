...
const createOrder = asyncHandler(async (req, res) => {
  const { deliveryInfo, orderItems, totalPrice, totalPriceAfterDiscount } =
    req.body;
  const { _id } = req.user;
  const user = await User.findById(_id);
  try {
    const khaltiRes = await axios.post(
      "https://a.khalti.com/api/v2/epayment/initiate/",
      {
        ...
        purchase_order_id: Date.now() + _id + totalPriceAfterDiscount,
        purchase_order_name: "Purchase from GetCosmo",
        customer_info: {
    ...
    if (khaltiRes.status !== 200)
      return res.status(500).json({ message: "Khalti api error" });
